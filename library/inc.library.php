...
{
	$conn = mysqli_connect('localhost', 'root', '', 'pustaka_2db');
	$struktur	= mysqli_query($conn, "SELECT * FROM $tabel");
	//$field		= mysqli_field_name($struktur, 0);
	$field = mysqli_field_seek($struktur, 0);
	//$panjang	= mysqli_fetch_len($struktur, 0);
	$panjang = mysqli_field_seek($struktur, 0);
