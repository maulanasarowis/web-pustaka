...
$pageSql = "SELECT * FROM peminjaman WHERE kd_siswa ='$kodeSiswa'";
$pageQry = mysqli_query($conn, $pageSql) or die("error paging: " . mysqli_error($conn));
...
<table class="table-list" width="800" border="0" cellspacing="1" cellpadding="2">
  ...
  $myQry = mysqli_query($conn, $mySql)  or die("Query 1 salah : " . mysqli_error($conn));
  $nomor = $mulai;
  while ($myData = mysqli_fetch_array($myQry)) {
    ...
    $infoQry = mysqli_query($conn, $infoSql)  or die("Query 2 salah : " . mysqli_error($conn));
    $infoData = mysqli_fetch_array($infoQry);
    if (mysqli_num_rows($infoQry) >= 1) {
    ...
    $info2Qry = mysqli_query($conn, $info2Sql)  or die("Query 3 salah : " . mysqli_error($conn));
    $info2Data = mysqli_fetch_array($info2Qry);
  ?>
    <tr>
