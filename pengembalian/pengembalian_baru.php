...
if (isset($_POST['btnSimpan'])) {
  ...
  $cekQry = mysqli_query($conn, $cekSql) or die("Gagal Query validasi " . mysqli_error($conn));
  ...
  if (count($pesanError) >= 1) {
    ...
    mysqli_query($conn, $statusSql) or die("Gagal query status " . mysqli_error($conn));
...
$myQry   = mysqli_query($conn, $mySql)  or die("Query ambil data salah : " . mysqli_error($conn));
    ...
    <tr>
      <td colspan="3"><strong>DAFTAR BUKU PINJAMAN</strong>
        <table class="table-list" width="740" border="0" cellspacing="1" cellpadding="2">
          ...
          $tmpQry = mysqli_query($conn, $tmpSql) or die("Gagal Query detil" . mysqli_error($conn));
          $nomor = 0;
