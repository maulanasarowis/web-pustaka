...
if (isset($_GET['Kode'])) {
  ...
  $mySql = "SELECT peminjaman.*, pengembalian.tgl_kembali, pengembalian.denda, siswa.nisn, siswa.nm_siswa FROM pengembalian 
        LEFT JOIN peminjaman ON pengembalian.no_pinjam = peminjaman.no_pinjam
        LEFT JOIN siswa ON peminjaman.kd_siswa = siswa.kd_siswa 
        WHERE pengembalian.no_pinjam='$Kode'";
...
<body onLoad="window.print()">
  ...
  <table class="table-list" width="600" border="0" cellspacing="0" cellpadding="2">
    ...
    $notaQry = mysqli_query($notaSql, $koneksidb)  or die("Query list salah : " . mysqli_error($conn));
    $nomor  = 0;
