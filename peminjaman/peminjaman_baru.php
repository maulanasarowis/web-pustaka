...
if (isset($_GET['Aksi'])) {
	if (trim($_GET['Aksi']) == "Hapus") {
		...
		mysqli_query($conn, $mySql) or die("Gagal menghapus tmp" . mysqli_error($conn));
	}
}
// =========================================================================
...
if (isset($_POST['btnTambah'])) {
	...
	if (count($pesanError) >= 1) {
		...
		mysqli_query($conn, $tmpSql) or die("Gagal Query tmp : " . mysqli_error($conn));
	}
}
...
if (isset($_POST['btnSimpan'])) {
	...
	$tmpQry = mysqli_query($conn, $tmpSql) or die("Gagal Query Tmp" . mysqli_error($conn));
	...
	if (count($pesanError) >= 1) {
		...
		mysqli_query($conn, $mySql) or die("Gagal query peminjaman " . mysqli_error($conn));
		...
		$tmpQry = mysqli_query($conn, $tmpSql) or die("Gagal Query baca Tmp" . mysqli_error($conn));
		while ($tmpData = mysqli_fetch_array($tmpQry)) {
			...
			mysqli_query($conn, $itemSql) or die("Gagal Query " . mysqli_error($conn));
		}

		// Kosongkan Tmp jika datanya sudah dipindah
		$hapusSql = "DELETE FROM tmp_pinjam WHERE kd_user='$userLogin' ";
		mysqli_query($conn, $hapusSql) or die("Gagal kosongkan tmp" . mysqli_error($conn));
	...
	<table width="900" cellspacing="1" class="table-list">
		...
		<tr>
			...
			<td><select name="cmbSiswa">
					...
					$bacaQry = mysqli_query($conn, $bacaSql) or die("Gagal Query" . mysqli_error($conn));
